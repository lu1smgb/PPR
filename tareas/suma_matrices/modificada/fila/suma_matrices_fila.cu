 ...
 * 
 * suma_matrices_fila.cu
 * Version del programa de suma de matrices en la que cada
 * hebra se encarga de una unica fila
 * 
 * ---------- TABLA DE TIEMPOS -----------
 * | Programa     | N      | Tiempo (s)  |
 * | ------------ | ------ | ----------- |
 ...
 * |              | 10000  | 0.008037    |
 * ---------------------------------------
 * Dispositivo: nVidia GA107M [GeForce RTX 3050 Mobile]
 * 
*/
...
{
    int row = blockDim.x * blockIdx.x + threadIdx.x; // Indice de fila
    if (row >= N) return; // Comprobacion de indice
    for (int column = 0; column < N; column++) { // Realiza la suma en toda la columna
