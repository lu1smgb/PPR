...
// Kernel que calcula el vector C version sin utilizar __shared__
__global__ void vectorialGPU_noshared(float *A, float *B, float *C, int N)
{
  int i = blockDim.x * blockIdx.x + threadIdx.x;
  if (i < N)
  {
    C[i] = 0;
    // Establecemos j y su valor limite
    int inicio = blockDim.x * blockIdx.x;
    int fin = inicio + blockDim.x;
...
float vectorialCPU(float *A, float *B, float *Cgpu, int N, int Bsize, int NBlocks)
{
  float *C = new float[N];
...
int main(int argc, char *argv[])
{
  ...
  timestamp = clock();
  vectorialGPU_noshared<<<numBlocks, M>>>(A, B, C, N);
  cudaDeviceSynchronize();

  // Terminamos benchmarking
  Tgpu = (clock() - timestamp) / CLOCKS_PER_SEC;
  cout << "Tiempo GPU (no shared): " << Tgpu << " seg" << endl;
  ...
  Tcpu = vectorialCPU(A, B, C, N, M, numBlocks);
  cout << "Tiempo CPU: " << Tcpu << " seg" << endl;

  // Mostramos ganancia de velocidad
  float S = Tcpu/Tgpu;
  cout << "Speedup (noshared): " << S << endl;
  ...
  cout << "Maximo GPU (noshared): " << mx << endl;
